...
class Fourier2D:
    ...
    def centerBanning(self, hp_filter="Ideal", ban_size=0):
        '''
        ...
        if ban_size <= 0:
            ...
            ban_size = int(ban_size * 0.15)
        
        if hp_filter == "Ideal":
            # -> Draw Ideal Circle
            center = (center_ban.shape[1]//2, center_ban.shape[0]//2)
            center_ban = cv.circle(center_ban, center, ban_size, 0, -1)
        elif hp_filter == "Gaussian":
            # -> Find Standard Deviation from Kernel Size
            gauss_std = lambda kernel_size : 0.3*((kernel_size-1)*0.5)+0.8
            # -> 1D Window Function
            winfunc_vert = scipy.signal.windows.gaussian(self.__IMG_HEIGHT, gauss_std(ban_size))
            winfunc_horz = scipy.signal.windows.gaussian(self.__IMG_WIDTH, gauss_std(ban_size))
            # -> 2D Window Function
            center_ban = winfunc_horz.reshape((1, self.__IMG_WIDTH)) * winfunc_vert.reshape((1, self.__IMG_HEIGHT)).T

        ### -> Banning Center
        fft_magnitude = fft_magnitude * (1-center_ban)
