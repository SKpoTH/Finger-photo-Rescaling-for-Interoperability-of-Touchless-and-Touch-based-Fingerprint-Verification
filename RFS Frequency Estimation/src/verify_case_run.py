...
from saves.respondFeaturesFigure import respondFeaturesFigure
from saves.freqResponse import freqResponse
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(first_idx, last_idx)):
        ...
        freq_subbands = fourierFeatures(ench_img, window_params, freq_params, 0)
        # - Find Ridge Size Representation
        input_freqRep, freq_response = sizeRepresentation(ench_img, freq_subbands, weak_cutoff=0.0)
        # ~> Save Median Scale
        medianResponseScale(input_freqRep, output_path + "Fingerphoto/Frequency Representation/", input_filenames[i])
        # ~> Save Respond Feature Figure
        respondFeaturesFigure(ench_img, freq_response, input_freqRep, save=True,
                              output_folder=output_path + "Fingerphoto/Repond Feautures Figure/",
                              output_filename=input_filenames[i])

        ### => Fingerprint Feature
        # - Obtain Fourier Transformation Features
        freq_subbands = fourierFeatures(ref_img, window_params, freq_params, 255)
        # - Find Ridge Size Representation
        ref_freqRep, freq_response = sizeRepresentation(ref_img, freq_subbands, weak_cutoff=0.0)
        # ~> Save Median Scale
        medianResponseScale(ref_freqRep, output_path + "Fingerprint/Frequency Representation/", ref_filename)
        # ~> Save Respond Feature Figure
        respondFeaturesFigure(ref_img, freq_response, ref_freqRep, save=True,
                              output_folder=output_path + "Fingerprint/Repond Feautures Figure/",
